...
int	init_data_sem(t_prog_dt *data)
{
	...
	sem_unlink("/end_eat");
	data->finish_eaten = sem_open("/end_eat", O_CREAT, 0777, data->n_philo);
	sem_unlink("/log_lck");
	data->log_lock = sem_open("/log_lck", O_CREAT, 0777, 1);
	sem_unlink("/end_lck");
	data->end_lock = sem_open("/end_lck", O_CREAT, 0777, 1);
	sem_unlink("/dth_lck");
	data->death_lock = sem_open("/dth_lck", O_CREAT, 0777, 1);
